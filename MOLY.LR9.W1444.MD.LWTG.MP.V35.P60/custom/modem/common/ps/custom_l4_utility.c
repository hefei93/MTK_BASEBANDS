...
#endif
// @lgx 1124
#if defined(__GEMINI__) && defined(__SIM_ME_LOCK__) && defined(__HQ_SML_LOCK_NV_EXT__)
...
{
	//add by zym,disable write imei by at command
    //return KAL_TRUE;
    return KAL_FALSE;
}
...
kal_uint8 custom_sms_fdn_check_da_only(void)
{
    ...
    return KAL_TRUE;
#else
    return KAL_TRUE;//add by chiguoqing for wuruijun
#endif
}
...
void custom_check_white_list(kal_bool * is_gsm_white_list,kal_bool * is_umts_white_list,kal_uint32 plmn_id)
{
    ...
    *is_umts_white_list =sbp_query_md_feature(SBP_AMR_WB_UMTS);    
    ...
    //add by zym,disable support AMR_WB in 3G
      *is_umts_white_list =KAL_FALSE;
    return ;
}
#endif
...
kal_uint8 pending_password_tbl_real[MAX_SIM_NUM]={ID_UNKNOWN,ID_UNKNOWN};
kal_bool sim1_illegal = KAL_FALSE;
kal_bool need_unlock = KAL_FALSE;
extern l4c_common_context_struct *l4c_comm_ptr_g;
extern kal_uint8 lock_state_sim1;
extern kal_uint8 lock_state_sim2;

void hq_simlock_set_SML_sim1_valid_unlock_sim2(kal_uint8 updated_sim, 
                                     smu_pending_password_id_enum updated_type, 
                                     smu_pending_password_id_enum *pending_password_tbl,
                                     kal_uint8 *need_unlock_table,
                                     kal_uint8 defaultSim)
{
    kal_uint8 i = 0;
	//defaultSim SIM1 or SIM2
    if (updated_sim == defaultSim)
    {
        kal_uint8 i;
		...
		if (updated_sim == SIM1){
	        ...
	        {
			kal_prompt_trace(MOD_SIM,"[SML]:need_unlock = %d; line = %d", need_unlock,__LINE__);
	            ...
	            {
			kal_prompt_trace(MOD_SIM,"[SML]:need_unlock = %d; line = %d", need_unlock,__LINE__);
	                pending_password_tbl_real[i] = ID_READY;
	                pending_password_tbl[i] = ID_READY;
	                need_unlock_table[i] = 1;
	                need_unlock = KAL_TRUE;
			kal_prompt_trace(MOD_SIM,"[SML]:SIM1 unlock SIM2");
	            }
	        }
		}
		else if(updated_sim == SIM2)
		{/* unlock all SIM if SIM2 is valid */
			...
			kal_prompt_trace(MOD_SIM,"[SML]:need_unlock = %d; line = %d", need_unlock,__LINE__);
	                ...
	                need_unlock_table[i] = 1;
	                need_unlock = KAL_TRUE;
			kal_prompt_trace(MOD_SIM,"[SML]:SIM2 unlock SIM1");
	            }
	        }
		}
    }
    ...
    kal_prompt_trace(MOD_SIM,"[SML]:need_unlock = %d; line = %d", need_unlock,__LINE__);
    pending_password_tbl_real[i] = updated_type;
}
void hq_simlock_set_SML_sim1_valid_unlock_sim2_ext(kal_uint8 updated_sim, 
                                     smu_pending_password_id_enum updated_type, 
                                     smu_pending_password_id_enum *pending_password_tbl,
                                     kal_uint8 *need_unlock_table,
                                     kal_bool sml_state_done,
                                     kal_uint8 defaultSim)
{
    kal_uint8 i = 0;
    if(updated_sim == SIM1)
    {
    kal_prompt_trace(MOD_SIM,"[SML]:sim1 is illegal");
    pending_password_tbl[updated_sim] = updated_type;
    pending_password_tbl_real[updated_sim] = updated_type;
    sim1_illegal =  KAL_TRUE;
    kal_prompt_trace(MOD_SIM,"[SML]:need_unlock = %d; line = %d", need_unlock,__LINE__);
    if(sml_state_done == KAL_TRUE)
    {
        kal_prompt_trace(MOD_SIM,"[SML]:need_unlock = %d; line = %d", need_unlock,__LINE__);
        kal_prompt_trace(MOD_SIM,"[SML]:ALL SIM SML states has been update, and no SIM is legal");
        pending_password_tbl_real[updated_sim] = updated_type;
        for(i = 0; i< MAX_SIM_NUM; i++)
        {
            pending_password_tbl[i] = pending_password_tbl_real[i];
        }
    }
    }
    else if(updated_sim == SIM2)
    {
    if(pending_password_tbl_real[0] == ID_READY)
    {
        kal_prompt_trace(MOD_SIM,"[SML]:i = %d; been unlock by SIM %d", i, l4c_gemini_get_actual_sim_id((sim_interface_enum)i));
        pending_password_tbl_real[updated_sim] = ID_READY;
        pending_password_tbl[updated_sim] = ID_READY;
        need_unlock_table[updated_sim] = 1;
        need_unlock = KAL_TRUE;
        kal_prompt_trace(MOD_SIM,"[SML]:SIM2 unlock itself");
        kal_prompt_trace(MOD_SIM,"[SML]:need_unlock = %d; line = %d", need_unlock,__LINE__);
        //break;
            return;
    }
    if(sml_state_done == KAL_FALSE)
    {
        kal_prompt_trace(MOD_SIM,"[SML]:sim1 SIM SML states has not been update");
        pending_password_tbl_real[updated_sim] = updated_type;
                                     ...
                                     kal_uint8 *need_unlock_table)
{
    #ifdef __HQ_SML_LOCK_NV_EXT__
    kal_uint8 i = 0;
    sim_interface_enum j = 0;
    kal_uint8 sim_inset_status[2]={0x01,0x02};
    kal_bool sml_state_done = KAL_TRUE;

    nvram_sml_context_struct_ext sml_buffer_ext;
    kal_uint8 type = SML_ALL_LOCK;
    hq_simlock_get_SML_EXT(&sml_buffer_ext);
        need_unlock = KAL_FALSE;
    kal_prompt_trace(MOD_SIM,"[SML]:need_unlock = %d; line = %d", need_unlock,__LINE__);
    for(i =0 ; i < MAX_SIM_NUM ; i++)
    {
        j = l4c_gemini_get_actual_sim_id((sim_interface_enum)i);
        kal_prompt_trace(MOD_SIM,"[SML]i = %d j = %d", i,j);
        if ((pending_password_tbl_real[i] != ID_PH_FSIM_PIN) &&
            (pending_password_tbl_real[i] != ID_PH_NET_PIN) &&
            (pending_password_tbl_real[i] != ID_PH_NETSUB_PIN) &&
            (pending_password_tbl_real[i] != ID_PH_SP_PIN) &&
            (pending_password_tbl_real[i] != ID_PH_CORP_PIN) &&
            (pending_password_tbl_real[i] != ID_PH_LINK_NS_SP_PIN) &&
            (pending_password_tbl_real[i] != ID_PH_LINK_SIM_C_PIN)&&
            (pending_password_tbl_real[i] != ID_READY)&&
            (pending_password_tbl_real[i] != ID_SIM_BLOCKED)&&
            (l4c_comm_ptr_g->dual_sim_inserted_status & sim_inset_status[i])&&
            (updated_sim != j))
        {
            kal_prompt_trace(MOD_SIM,"[SML]:need_unlock = %d; line = %d", need_unlock,__LINE__);
            sml_state_done = KAL_FALSE;
            break;
        }
    }

    kal_prompt_trace(MOD_SIM,"[SML]:sml_state_done = %d; updated_type = %d; i = %d; j = %d", sml_state_done, updated_type, i-1, j);
    kal_prompt_trace(MOD_SIM,"[SML]:updated_sim = %d; l4c_comm_ptr_g->dual_sim_inserted_status = %d", updated_sim, l4c_comm_ptr_g->dual_sim_inserted_status);
    kal_prompt_trace(MOD_SIM,"[SML]:need_unlock = %d; line = %d", need_unlock,__LINE__);
    #else
        kal_bool need_unlock = KAL_FALSE;
    #endif
...
#ifdef __HQ_SML_LOCK_NV_EXT__
    // TODO add sim lock feature
    if(sml_buffer_ext.sml_type[0] == 0x01)
    {
        type = SML_SIM1_SIM2_UNLOCK;
    }
    else if(sml_buffer_ext.sml_type[0] == 0x02)
    {
        type = SML_SIM1_SIM2_VERIFY;
    }
    else if(sml_buffer_ext.sml_type[0] == 0x03)
    {
        type = SML_SIM1_SIM2_VERIFY_UNLOCK;
    }
	else if(sml_buffer_ext.sml_type[0] == 0x04)
	{
	    type = SML_NO_SIM1_SIM2_VERIFY;
	}
	else if(sml_buffer_ext.sml_type[0] == 0x05)
	{
	    type = SML_SIM1_SIM2_NO_LINK;
	}
	else if(sml_buffer_ext.sml_type[0] == 0x06)
	{
	    type = SML_SIM1_SIM2_HONGKONG;
	}
    else
    {
	...
	kal_uint8 defaultSim = sml_buffer_ext.code_other[0];
#endif
    switch (updated_type)
    {
        case ID_READY:
	    ...
	    switch (type)
	    {
		case SML_SIM1_SIM2_UNLOCK:
		    kal_prompt_trace(MOD_SIM,"[SML]:in SML_SIM1_SIM2_UNLOCK line = %d",__LINE__);
		    if(!lock_state_sim1)
		    {
		        /* default each SML is independent */
			kal_prompt_trace(MOD_SIM,"[SML]:need_unlock = %d; line = %d", need_unlock,__LINE__);
		    ...
		    hq_simlock_set_SML_sim1_valid_unlock_sim2(
						updated_sim,
						updated_type,
						...
						defaultSim);
		    break;
		case SML_SIM1_SIM2_VERIFY:
		case SML_NO_SIM1_SIM2_VERIFY:
		    kal_prompt_trace(MOD_SIM,"[SML]:in SML_SIM1_SIM2_VERIFY line = %d",__LINE__);
            if(lock_state_sim1)
		    {
		        /* default each SML is independent */
			kal_prompt_trace(MOD_SIM,"[SML]:need_unlock = %d; line = %d", need_unlock,__LINE__);
		    ...
		    hq_simlock_set_SML_sim1_valid_unlock_sim2(
						...
						defaultSim);
		break;
		case SML_SIM1_SIM2_VERIFY_UNLOCK:
		    kal_prompt_trace(MOD_SIM,"[SML]:in SML_SIM1_SIM2_VERIFY_UNLOCK line = %d",__LINE__);
		    hq_simlock_set_SML_sim1_valid_unlock_sim2(
						updated_sim,
						updated_type,
						pending_password_tbl,
						...
						defaultSim);
		break;
		case SML_SIM1_SIM2_NO_LINK:
		case SML_SIM1_SIM2_HONGKONG:
		default:
		    /* default each SML is independent */
		    pending_password_tbl[updated_sim] = updated_type;
		    pending_password_tbl_real[i] = updated_type;
		...
		switch(type){
		    case SML_SIM1_SIM2_UNLOCK:
		        ...
		        hq_simlock_set_SML_sim1_valid_unlock_sim2_ext(
						updated_sim,
						updated_type,
						pending_password_tbl,
						need_unlock_table,
		    ...
		    case SML_NO_SIM1_SIM2_VERIFY:
		        kal_prompt_trace(MOD_SIM,"[SML]:in SML_NO_SIM1_SIM2_VERIFY line = %d",__LINE__);
		        kal_prompt_trace(MOD_SIM,"[SML]:sim1 is = %d",is_sim_inserted(l4c_gemini_get_switched_sim_id(0)));
				kal_prompt_trace(MOD_SIM,"[SML]:i = %d; been unlock by SIM %d", i, l4c_gemini_get_actual_sim_id((sim_interface_enum)i));
				if( (is_sim_inserted(l4c_gemini_get_switched_sim_id(0)) == SMU_SIM_NOT_INSERTED) && (updated_sim == SIM2)){
				pending_password_tbl_real[updated_sim] = ID_READY;
				pending_password_tbl[updated_sim] = ID_READY;
				need_unlock_table[updated_sim] = 1;
				need_unlock = KAL_TRUE;
