...
kal_uint8 lock_state_sim1 = KAL_FALSE;
kal_uint8 lock_state_sim2 = KAL_FALSE;
#endif
#ifdef __HQ_SML_LOCK_NV_ANTI_BREAK__
extern void hq_simlock_set_SML_Unlock_Anti_Break(sml_cat_enum cat, kal_uint8 sim_id);
                      ...
                      kal_uint8 source)
{
    ...
    #ifdef __HQ_SML_LOCK_NV_EXT__
    kal_uint8 sim = l4c_gemini_get_actual_sim_id((sim_interface_enum)source);
    #endif
	...
	#ifdef __HQ_SML_ALWAYS_UNLOCK__
	meta->state = SML_STATE_UNLOCK;
	{
	#ifdef __HQ_SML_LOCK_NV_EXT__
	...
	kal_prompt_trace(MOD_SIM,"[SML]:type = %d. sim = %d. defaultSim = %d",type , sim , defaultSim);
	switch(type){
	    case SML_SIM1_SIM2_UNLOCK:
            kal_prompt_trace(MOD_SIM,"[SML]:SML_SIM1_SIM2_UNLOCK sim=%d", sim);
	    if(sim==defaultSim)//sim1 first unlock, sim2 unlock
	    {
	        hq_simlock_set_SML_Unlock_Anti_Break(cat, 1);//sim1
	        hq_simlock_set_SML_Unlock_Anti_Break(cat, 2);//sim2
	        lock_state_sim1 = KAL_TRUE;
            kal_prompt_trace(MOD_SIM,"[SML]:SML_SIM1_SIM2_UNLOCK  sim1 and sim2");
	    }
	    else
	    {
            kal_prompt_trace(MOD_SIM,"[SML]:SML_SIM1_SIM2_UNLOCK  sim1 or sim2");
			if(sim==SIM1){
			    hq_simlock_set_SML_Unlock_Anti_Break(cat, 1);
			}
			else
			{
			    hq_simlock_set_SML_Unlock_Anti_Break(cat, 2);
			}
	    }
            //ASSERT(0);
	    break;
	    ...
	    sim++;
	    hq_simlock_set_SML_Unlock_Anti_Break(cat, sim);
	    lock_state_sim1 = KAL_TRUE;
	    break;
    ...
    {
#ifdef __HQ_SML_ALWAYS_RETRY__
        if(meta->retry_count != 0xFF){
	    meta->retry_count--;
	    sim++;
	    hq_simlock_set_SML_Retry_Count_Anti_Break(cat, meta->retry_count, sim);
        }
#else
	    meta->retry_count--;
#endif
