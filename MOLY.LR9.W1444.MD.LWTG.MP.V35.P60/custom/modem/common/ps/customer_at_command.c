...
kal_bool custom_command_hdlr(char *full_cmd_string)
{
    ...
    else if (strcmp(cmd_name, "QSIM1") == 0)
    {
        extern void hq_simlock_query_sml_info(kal_uint8 sim_id, sml_context_struct* sml_static_buffer);
	...
	memset(&sml_static_buffer,0x00,sizeof(sml_context_struct));
        hq_simlock_query_sml_info(1, &sml_static_buffer);
	kal_prompt_trace(MOD_SIM, "yangfengfeng[sim1]: receive full_cmd_string =%s", full_cmd_string);
	...
	for(cat = SML_CAT_N; cat < SML_CAT_SIZE; cat++)
        ...
        if(isLocked == 1)
        {
            ...
            //kal_prompt_trace(MOD_SIM, "yangfengfeng: receive cmd =%s ; flag is %x ", cmd_name, read_buffer);
            }
	...
	//kal_prompt_trace(MOD_NVRAM, "yangfengfeng[sim1]: info_buffer is === >> %s", info_buffer);
	rmmi_write_to_uart((kal_uint8*)info_buffer, strlen(info_buffer), KAL_FALSE);
	sprintf(buffer, "OK");
	rmmi_write_to_uart((kal_uint8*)buffer, strlen(buffer), KAL_TRUE);
	return KAL_TRUE;
    }
    else if (strcmp(cmd_name, "QSIM2") == 0)
    {
        extern void hq_simlock_query_sml_info(kal_uint8 sim_id, sml_context_struct* sml_static_buffer);
	char info_buffer[MAX_UART_LEN+1];
	int NumOfPlmn,i;
	...
	sml_context_struct sml_static_buffer;
	memset(&sml_static_buffer,0x00,sizeof(sml_context_struct));
        hq_simlock_query_sml_info(2, &sml_static_buffer);
	kal_prompt_trace(MOD_SIM, "yangfengfeng[sim2]: receive full_cmd_string =%s", full_cmd_string);
	sml_cat_enum cat = 0xff;
	for(cat = SML_CAT_N; cat < SML_CAT_SIZE; cat++)
        {
            if(sml_static_buffer.cat[cat].state == 0x01)
        ...
        if(isLocked == 1)
        {
			...
			//add by zym
			NumOfPlmn = 1;
	    sprintf(info_buffer, "+QSIM2:%02x;",sml_static_buffer.cat[cat].state);
	    sprintf(info_buffer+strlen(info_buffer), "%02x;",NumOfPlmn);
            if(cat == SML_CAT_N)
            {
                for(i = 0; i < NumOfPlmn*SML_SIZE_OF_CAT_N; i=i+SML_SIZE_OF_CAT_N)
        ...
        {
            sprintf(info_buffer, "+QSIM2:%02x;",0x00);
        }
	//kal_prompt_trace(MOD_NVRAM, "yangfengfeng[sim2]: info_buffer is === >> %s", info_buffer);
	rmmi_write_to_uart((kal_uint8*)info_buffer, strlen(info_buffer), KAL_FALSE);
	sprintf(buffer, "OK");
	rmmi_write_to_uart((kal_uint8*)buffer, strlen(buffer), KAL_TRUE);
	return KAL_TRUE;
    }
    else if (strcmp(cmd_name, "SMLUN1") == 0)
    {
    extern void hq_simlock_query_sml_info(kal_uint8 sim_id, sml_context_struct* sml_static_buffer);
    extern void hq_simlock_set_sml_info(kal_uint8 sim_id, sml_context_struct* sml_static_buffer);
	...
	sml_cat_enum cat_real = SML_CAT_N;
	for(cat = SML_CAT_N; cat < SML_CAT_SIZE; cat++)
    {
    ...
    sprintf(info_buffer, "+SMLUN1:%02x;",sml_static_buffer.cat[cat_real].retry_count);
    hq_simlock_set_sml_info(1, &sml_static_buffer);

	kal_prompt_trace(MOD_SIM, "yangfengfeng: reset sml nvram to unlock sim1 ===========");
    rmmi_write_to_uart((kal_uint8*)info_buffer, strlen(info_buffer), KAL_TRUE);
    sprintf(buffer, "OK");
    rmmi_write_to_uart((kal_uint8*)buffer, strlen(buffer), KAL_TRUE);
    return KAL_TRUE;
    }
    else if (strcmp(cmd_name, "SMLUN2") == 0)
    {
    extern void hq_simlock_query_sml_info(kal_uint8 sim_id, sml_context_struct* sml_static_buffer);
    extern void hq_simlock_set_sml_info(kal_uint8 sim_id, sml_context_struct* sml_static_buffer);
    ...
    sprintf(info_buffer, "+SMLUN2:%02x;",sml_static_buffer.cat[cat_real].retry_count);

    hq_simlock_set_sml_info(2, &sml_static_buffer);

	kal_prompt_trace(MOD_SIM, "yangfengfeng: reset sml nvram to unlock sim2 ===========");
    rmmi_write_to_uart((kal_uint8*)info_buffer, strlen(info_buffer), KAL_TRUE);
    sprintf(buffer, "OK");
    rmmi_write_to_uart((kal_uint8*)buffer, strlen(buffer), KAL_TRUE);
    return KAL_TRUE;
    }
    else if (strcmp(cmd_name, "SMLUN") == 0)
    {
    extern void hq_simlock_query_sml_info(kal_uint8 sim_id, sml_context_struct* sml_static_buffer);
    extern void hq_simlock_set_sml_info(kal_uint8 sim_id, sml_context_struct* sml_static_buffer);
    ...
    sprintf(info_buffer, "+SMLUN:%02x;",sml_static_buffer.cat[cat_real].retry_count);
    hq_simlock_set_sml_info(1, &sml_static_buffer);
	...
	kal_prompt_trace(MOD_SIM, "yangfengfeng: reset sml nvram to unlock sim ===========");
    rmmi_write_to_uart((kal_uint8*)info_buffer, strlen(info_buffer), KAL_TRUE);
    sprintf(buffer, "OK");
    rmmi_write_to_uart((kal_uint8*)buffer, strlen(buffer), KAL_TRUE);
    return KAL_TRUE;
    ...
    #endif
    #ifdef __HQ_SML_LOCK_NV_EXT__
    else if (strcmp(cmd_name, "QSMLE") == 0)
    {
        extern void hq_simlock_get_SML_EXT(nvram_sml_context_struct_ext *sml_buffer_ext);
	char info_buffer[MAX_UART_LEN+1];
	...
	kal_prompt_trace(MOD_SIM, "yangfengfeng: receive full_cmd_string =%s", full_cmd_string);
	//kal_prompt_trace(MOD_SIM, "yangfengfeng: receive cmd =%s ; flag is %x ", cmd_name, read_buffer);

	sprintf(info_buffer,"+QSMLE:");
	sprintf(info_buffer+strlen(info_buffer),"%02x;",sml_buffer_ext.sml_type[0]);
	sprintf(info_buffer+strlen(info_buffer),"%02x,%02x;",sml_buffer_ext.need_pop[0], sml_buffer_ext.need_pop[1]);
	sprintf(info_buffer+strlen(info_buffer),"%02x;", sml_buffer_ext.can_cancel_pop[0]);
	sprintf(info_buffer+strlen(info_buffer),"%02x,%02x,%02x,%02x;",sml_buffer_ext.unlock_time[0], 
                sml_buffer_ext.unlock_time[1],sml_buffer_ext.unlock_time[2],sml_buffer_ext.unlock_time[3]);
    sprintf(info_buffer+strlen(info_buffer),"%02x;", sml_buffer_ext.unlock_pop[0]);
    sprintf(info_buffer+strlen(info_buffer),"%02x;", sml_buffer_ext.code_other[0]);
    sprintf(info_buffer+strlen(info_buffer),"%02x%02x%02x%02x;", sml_buffer_ext.code_other[1], sml_buffer_ext.code_other[2], sml_buffer_ext.code_other[3], sml_buffer_ext.code_other[4]);

	//kal_prompt_trace(MOD_NVRAM, "yangfengfeng[orange]: info_buffer is === >> %s", info_buffer);
	rmmi_write_to_uart((kal_uint8*)info_buffer, strlen(info_buffer), KAL_TRUE);
	sprintf(buffer, "OK");
	rmmi_write_to_uart((kal_uint8*)buffer, strlen(buffer), KAL_TRUE);
	return KAL_TRUE;
    }
    else if (strcmp(cmd_name, "QSMLEK") == 0)
    {
        extern void hq_simlock_get_SML_EXT(nvram_sml_context_struct_ext *sml_buffer_ext);
	char info_buffer[MAX_UART_LEN+1];
	nvram_sml_context_struct_ext sml_buffer_ext;
    ...
    else if (strcmp(cmd_name, "SSMLE") == 0)
    {
    extern void hq_simlock_get_SML_EXT(nvram_sml_context_struct_ext *sml_buffer_ext);
	extern void hq_simlock_set_SML_EXT(nvram_sml_context_struct_ext *sml_buffer_ext);
	char info_buffer[MAX_UART_LEN+1];
	nvram_sml_context_struct_ext sml_buffer_ext;
	memset(&sml_buffer_ext,0x00,sizeof(nvram_sml_context_struct_ext));
	hq_simlock_get_SML_EXT(&sml_buffer_ext);
	kal_prompt_trace(MOD_SIM, "yangfengfeng: receive full_cmd_string =%s", full_cmd_string);
	//kal_prompt_trace(MOD_SIM, "yangfengfeng: receive cmd =%s ; flag is %x ", cmd_name, read_buffer);
    sml_buffer_ext.code_other[0] = 0X00;
	//kal_prompt_trace(MOD_NVRAM, "yangfengfeng[orange]: info_buffer is === >> %s", info_buffer);
	//rmmi_write_to_uart((kal_uint8*)info_buffer, strlen(info_buffer), KAL_TRUE);
	hq_simlock_set_SML_EXT(&sml_buffer_ext);
	sprintf(buffer, "OK");
	rmmi_write_to_uart((kal_uint8*)buffer, strlen(buffer), KAL_TRUE);
	return KAL_TRUE;
    }
    else if (strcmp(cmd_name, "SSMLE1") == 0)
    {
    extern void hq_simlock_get_SML_EXT(nvram_sml_context_struct_ext *sml_buffer_ext);
	extern void hq_simlock_set_SML_EXT(nvram_sml_context_struct_ext *sml_buffer_ext);
	char info_buffer[MAX_UART_LEN+1];
	...
	kal_prompt_trace(MOD_SIM, "yangfengfeng: receive full_cmd_string =%s", full_cmd_string);
	//kal_prompt_trace(MOD_SIM, "yangfengfeng: receive cmd =%s ; flag is %x ", cmd_name, read_buffer);
    sml_buffer_ext.code_other[0] = 0X01;
	//kal_prompt_trace(MOD_NVRAM, "yangfengfeng[orange]: info_buffer is === >> %s", info_buffer);
	//rmmi_write_to_uart((kal_uint8*)info_buffer, strlen(info_buffer), KAL_TRUE);
	hq_simlock_set_SML_EXT(&sml_buffer_ext);
	sprintf(buffer, "OK");
