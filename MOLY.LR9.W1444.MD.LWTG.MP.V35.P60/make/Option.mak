...
#yangfengfeng add sml lock support start
ifdef HQ_SML_LOCK_SUPPORT
ifeq ($(strip $(HQ_SML_LOCK_SUPPORT)),TRUE)
    COM_DEFS += __HQ_SML_LOCK_NV_ANTI_BREAK__# usr anti break nv
    COM_DEFS += __HQ_NW_LOCK_ENLARGE_COUNT__ # support mccmnc 50
#    COM_DEFS +=__HQ_SML_LOCK_NV_MCCMNC_GID1__ # lock mccmnc+gid1
    COM_DEFS +=__HQ_SML_ALWAYS_RETRY__ #when input wrong sp , always can retry
#    COM_DEFS +=__MMI_SML_UNLOCK_AFTER_VERIFIED__ # input np code to unlock
    COM_DEFS +=__HQ_SML_ALWAYS_UNLOCK__ #cleared mccmnc lock status
   COM_DEFS +=__HQ_SML_LOCK_NV_EXT__
  ifdef GEMINI
    COM_DEFS +=__LINK_SML__ #open for dual sim
  endif
  endif
endif
#yangfengfeng add sml lock support end
