...
void phb_get_info(ilm_struct *ilm_ptr, control_block_type *cblk)
{
    ...
    while (phb_query_count[curr_sim] >=0 && phb_sim_info_tbl[phb_query_count[curr_sim]].primary_ID)
    {
        ...
        if (phb_sim_info_tbl[phb_query_count[curr_sim]].is_usim != PHB_INFO_CHK_TOTAL)
        {
            ...
            {
                  /* modified by maolikui at 2015-09-29 to resolve SIM issue start */
                 if (usim && phb_sim_info_tbl[phb_query_count[curr_sim]].primary_ID == FILE_ADN_IDX &&
                   (phb_ptr->data_desc[DATA_DESC_PBR].record_num == 0 || phb_ptr->data_desc[DATA_DESC_PBR].is_support == KAL_FALSE))

                 {

                    kal_prompt_trace(MOD_PHB,"USIM and file id  : FILE_ADN_IDX");

                    kal_prompt_trace(MOD_PHB,"record_num:%d",phb_ptr->data_desc[DATA_DESC_PBR].record_num);

                    kal_prompt_trace(MOD_PHB,"pbr is support:%d",phb_ptr->data_desc[DATA_DESC_PBR].is_support);

 		  /* modified by maolikui at 2015-09-29 to resolve SIM issue end  */
