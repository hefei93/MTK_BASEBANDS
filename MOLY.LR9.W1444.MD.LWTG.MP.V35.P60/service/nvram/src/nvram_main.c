...
#ifdef __HQ_SML_LOCK_NV_ANTI_BREAK__ 
#define	S16 kal_int16
#include "ps_public_enum.h"
#include "custom_nvram_sec.h"
#if 0//def __HQ_SML_LOCK_NV_MCCMNC_GID1__
#define NAME_SIMLOCK_CAT_DST SML_CAT_SP
#define SIZE_SIMLOCK_CAT_DST SML_SIZE_OF_CAT_SP
#define NAME_CODE_CAT code_cat_sp
#else
#define NAME_SIMLOCK_CAT_DST SML_CAT_N
#define SIZE_SIMLOCK_CAT_DST SML_SIZE_OF_CAT_N
#define NAME_CODE_CAT code_cat_n
#endif
#define CODE_OTHER  {0x00}
void hq_simlock_fill_in_SML(void)
{
	S16 NumOfPlmn;
	S16 i ,error;
	kal_uint8 other_code[] = CODE_OTHER;
	//sml_context_struct sml_static_buffer;
	sml_context_struct sml_static_backup_buffer;
	//sml_context_struct sml_static_buffer_sim2;
	sml_context_struct sml_static_backup_buffer_sim2;

	//memset(&sml_static_buffer,0x00,sizeof(sml_context_struct));
	memset(&sml_static_backup_buffer,0x00,sizeof(sml_context_struct));
	//memset(&sml_static_buffer_sim2,0x00,sizeof(sml_context_struct));
	memset(&sml_static_backup_buffer_sim2,0x00,sizeof(sml_context_struct));
	//ASSERT(0);
	#if 0//def __HQ_SML_LOCK_NV_ORANGE__
	gid1_context_struct gid1_mccmnc_buffer;
	nvram_external_read_data(NVRAM_EF_GID1_LID,1, (void*)(&gid1_mccmnc_buffer), sizeof(gid1_context_struct));
	#endif

//	kal_prompt_trace(MOD_SIM,"yangfengfeng:hq_simlock_fill_in_SML  start");
//	kal_prompt_trace(MOD_SIM,"yangfengfeng:NAME_SIMLOCK_CAT_DST is %d " , NAME_SIMLOCK_CAT_DST);
//	nvram_external_read_data(NVRAM_EF_SML_LID, 1, (void*)(&sml_static_backup_buffer),NVRAM_EF_SML_SIZE);

	nvram_external_read_data(NVRAM_EF_SML_ANTI_BREAK_LID, 1, (void*)(&sml_static_backup_buffer),NVRAM_EF_SML_ANTI_BREAK_SIZE);
	//nvram_external_read_data(NVRAM_EF_SML_LID,1, (void*)(&sml_static_buffer), NVRAM_EF_SML_SIZE);

	//copy state num
	//sml_static_buffer.cat[NAME_SIMLOCK_CAT_DST].state = sml_static_backup_buffer.cat[0].state;
	//sml_static_buffer.cat[NAME_SIMLOCK_CAT_DST].num = sml_static_backup_buffer.cat[0].num;
	//sml_static_buffer.key[NAME_SIMLOCK_CAT_DST].state = sml_static_backup_buffer.key[0].state;
//	kal_prompt_trace(MOD_SIM,"yangfengfeng:sml_static_buffer  cat state is %d " , sml_static_buffer.cat[NAME_SIMLOCK_CAT_DST].state);
//	kal_prompt_trace(MOD_SIM,"yangfengfeng:sml_static_buffer  cat num is %d " , sml_static_buffer.cat[NAME_SIMLOCK_CAT_DST].num);
//	kal_prompt_trace(MOD_SIM,"yangfengfeng:sml_static_buffer  key state is %d " , sml_static_buffer.key[NAME_SIMLOCK_CAT_DST].state);

	//copy key
	//memcpy((void*)(&(sml_static_buffer.key[NAME_SIMLOCK_CAT_DST].key[0])),
	//		(void*)(&(sml_static_backup_buffer.key[0].key[0])),
	//		SML_MAX_SUPPORT_KEY_LEN*sizeof(kal_uint8));
	//NumOfPlmn = sml_static_buffer.cat[NAME_SIMLOCK_CAT_DST].num;
//	kal_prompt_trace(MOD_SIM,"yangfengfeng:hq_customize_plmn_and_other_in_sml  start");
	//for(i = 0; i < NumOfPlmn; i++)
	//{
//copy mccmnc
//		kal_prompt_trace(MOD_SIM,"yangfengfeng:copy mccmnc ");
	//	memcpy(
	//		&(sml_static_buffer.NAME_CODE_CAT[i*SIZE_SIMLOCK_CAT_DST]),
	//		&(sml_static_backup_buffer.code_cat_n[i*SML_SIZE_OF_CAT_N]),
	//		SML_SIZE_OF_CAT_N
	//		);
#if 0//def __HQ_SML_LOCK_NV_MCCMNC_GID1__
//copy gid1
//		kal_prompt_trace(MOD_SIM,"yangfengfeng:copy gid1 ");
		memcpy(
			&(sml_static_buffer.NAME_CODE_CAT[i*SIZE_SIMLOCK_CAT_DST + SML_SIZE_OF_CAT_N]),
			other_code,
			sizeof(other_code)/sizeof(kal_uint8)
			);
#endif
	//}
	nvram_external_write_data(NVRAM_EF_SML_LID, 1, (void*)(&sml_static_backup_buffer), NVRAM_EF_SML_SIZE);

	//sim2
	nvram_external_read_data(NVRAM_EF_SML_ANTI_BREAK_LID, 2, (void*)(&sml_static_backup_buffer_sim2),NVRAM_EF_SML_ANTI_BREAK_SIZE);
	//nvram_external_read_data(NVRAM_EF_SML_LID,2, (void*)(&sml_static_buffer_sim2), NVRAM_EF_SML_SIZE);
	//copy state num
	//sml_static_buffer_sim2.cat[NAME_SIMLOCK_CAT_DST].state = sml_static_backup_buffer_sim2.cat[0].state;
	//sml_static_buffer_sim2.cat[NAME_SIMLOCK_CAT_DST].num = sml_static_backup_buffer_sim2.cat[0].num;
	//sml_static_buffer_sim2.key[NAME_SIMLOCK_CAT_DST].state = sml_static_backup_buffer_sim2.key[0].state;
//	kal_prompt_trace(MOD_SIM,"yangfengfeng:sml_static_buffer  cat state is %d " , sml_static_buffer.cat[NAME_SIMLOCK_CAT_DST].state);
//	kal_prompt_trace(MOD_SIM,"yangfengfeng:sml_static_buffer  cat num is %d " , sml_static_buffer.cat[NAME_SIMLOCK_CAT_DST].num);
//	kal_prompt_trace(MOD_SIM,"yangfengfeng:sml_static_buffer  key state is %d " , sml_static_buffer.key[NAME_SIMLOCK_CAT_DST].state);

	//copy key
	//memcpy((void*)(&(sml_static_buffer_sim2.key[NAME_SIMLOCK_CAT_DST].key[0])),
	//		(void*)(&(sml_static_backup_buffer_sim2.key[0].key[0])),
	//		SML_MAX_SUPPORT_KEY_LEN*sizeof(kal_uint8));

	//NumOfPlmn = sml_static_buffer_sim2.cat[NAME_SIMLOCK_CAT_DST].num;
//	kal_prompt_trace(MOD_SIM,"yangfengfeng:hq_customize_plmn_and_other_in_sml  start");
	//for(i = 0; i < NumOfPlmn; i++)
	//{
//copy mccmnc
//		kal_prompt_trace(MOD_SIM,"yangfengfeng:copy mccmnc ");
	//	memcpy(
	//		&(sml_static_buffer_sim2.NAME_CODE_CAT[i*SIZE_SIMLOCK_CAT_DST]),
	//		&(sml_static_backup_buffer_sim2.code_cat_n[i*SML_SIZE_OF_CAT_N]),
	//		SML_SIZE_OF_CAT_N
	//		);
	//}
	nvram_external_write_data(NVRAM_EF_SML_LID, 2, (void*)(&sml_static_backup_buffer_sim2), NVRAM_EF_SML_SIZE);
//	kal_prompt_trace(MOD_SIM,"yangfengfeng:hq_simlock_fill_in_SML  end");
}
void hq_simlock_set_SML_Unlock_Anti_Break(sml_cat_enum cat, kal_uint8 sim_id)
{
	sml_context_struct sml_static_backup_buffer;
	nvram_external_read_data(NVRAM_EF_SML_ANTI_BREAK_LID, sim_id, (void*)(&sml_static_backup_buffer), NVRAM_EF_SML_SIZE);
	//reset sml nvram .
	sml_static_backup_buffer.cat[cat].state = 0x02;
	nvram_external_write_data(NVRAM_EF_SML_ANTI_BREAK_LID, sim_id, (void*)(&sml_static_backup_buffer), NVRAM_EF_SML_SIZE);
}
void hq_simlock_set_SML_Retry_Count_Anti_Break(sml_cat_enum cat, kal_uint8 retry_count, kal_uint8 sim_id)
{
	sml_context_struct sml_static_backup_buffer;
	nvram_external_read_data(NVRAM_EF_SML_ANTI_BREAK_LID, sim_id, (void*)(&sml_static_backup_buffer), NVRAM_EF_SML_SIZE);
        //ASSERT(0);
	sml_static_backup_buffer.cat[cat].retry_count= retry_count;
	nvram_external_write_data(NVRAM_EF_SML_ANTI_BREAK_LID, sim_id, (void*)(&sml_static_backup_buffer), NVRAM_EF_SML_SIZE);
}
void hq_simlock_clear_SML(void)
{
	sml_context_struct sml_static_buffer;
	memset(&sml_static_buffer,0x00,sizeof(sml_context_struct));
	nvram_external_read_data(NVRAM_EF_SML_LID,1, (void*)(&sml_static_buffer), NVRAM_EF_SML_SIZE);
	...
	for(cat = SML_CAT_N; cat < SML_CAT_SIZE; cat++)
        {
	    sml_static_buffer.cat[cat].state = 0x02;
        }
	nvram_external_write_data(NVRAM_EF_SML_LID, 1, (void*)(&sml_static_buffer), NVRAM_EF_SML_SIZE);
}
void hq_simlock_clear_SML_Anti_Break(void)
{
	sml_context_struct sml_static_buffer;
	memset(&sml_static_buffer,0x00,sizeof(sml_context_struct));
	nvram_external_read_data(NVRAM_EF_SML_ANTI_BREAK_LID,1, (void*)(&sml_static_buffer), NVRAM_EF_SML_ANTI_BREAK_SIZE);
	//reset sml nvram .
	sml_cat_enum cat = SML_CAT_N;
	for(cat = SML_CAT_N; cat < SML_CAT_SIZE; cat++)
        {
	...
	nvram_external_write_data(NVRAM_EF_SML_ANTI_BREAK_LID, 1, (void*)(&sml_static_buffer), NVRAM_EF_SML_ANTI_BREAK_SIZE);
}
//for sim2
void hq_simlock_clear_SML_SIM2(void)
{
	sml_context_struct sml_static_buffer;
	memset(&sml_static_buffer,0x00,sizeof(sml_context_struct));
	nvram_external_read_data(NVRAM_EF_SML_LID,2, (void*)(&sml_static_buffer), NVRAM_EF_SML_SIZE);
	//reset sml nvram .
	sml_cat_enum cat = SML_CAT_N;
	for(cat = SML_CAT_N; cat < SML_CAT_SIZE; cat++)
        {
	...
	nvram_external_write_data(NVRAM_EF_SML_LID, 2, (void*)(&sml_static_buffer), NVRAM_EF_SML_SIZE);
}
//for sim2
void hq_simlock_clear_SML_Anti_Break_SIM2(void)
{
	sml_context_struct sml_static_buffer;
	memset(&sml_static_buffer,0x00,sizeof(sml_context_struct));
	nvram_external_read_data(NVRAM_EF_SML_ANTI_BREAK_LID,2, (void*)(&sml_static_buffer), NVRAM_EF_SML_ANTI_BREAK_SIZE);
	//reset sml nvram .
	sml_cat_enum cat = SML_CAT_N;
	for(cat = SML_CAT_N; cat < SML_CAT_SIZE; cat++)
        {
	...
	nvram_external_write_data(NVRAM_EF_SML_ANTI_BREAK_LID, 2, (void*)(&sml_static_buffer), NVRAM_EF_SML_ANTI_BREAK_SIZE);
}
void hq_simlock_query_sml_info(kal_uint8 sim_id, sml_context_struct* sml_static_buffer)
{
	nvram_external_read_data(NVRAM_EF_SML_ANTI_BREAK_LID, sim_id, (void*)(sml_static_buffer),NVRAM_EF_SML_ANTI_BREAK_SIZE);
}
void hq_simlock_set_sml_info(kal_uint8 sim_id, sml_context_struct* sml_static_buffer)
{
	nvram_external_write_data(NVRAM_EF_SML_ANTI_BREAK_LID, sim_id, (void*)(sml_static_buffer), NVRAM_EF_SML_ANTI_BREAK_SIZE);
}

void hq_simlock_unlock_SML_SIM1(void)
{
	hq_simlock_clear_SML_Anti_Break();
	hq_simlock_clear_SML();
}
void hq_simlock_unlock_SML_SIM2(void)
{
	hq_simlock_clear_SML_Anti_Break_SIM2();
	hq_simlock_clear_SML_SIM2();
}
void hq_simlock_unlock_SML(void)
{
	hq_simlock_clear_SML_Anti_Break();
	hq_simlock_clear_SML();
	hq_simlock_clear_SML_Anti_Break_SIM2();
...
#ifdef __HQ_SML_LOCK_NV_EXT__
/*
nvram_sml_context_struct_ext *sml_static_buffer_ext;
void hq_simlock_fill_in_SML_EXT(void)
{
	nvram_sml_context_struct_ext sml_buffer_ext;
	memset(&sml_buffer_ext,0x00,sizeof(nvram_sml_context_struct_ext));
	kal_prompt_trace(MOD_SIM,"yangfengfeng:hq_simlock_fill_in_SML_EXT");
	nvram_external_read_data(NVRAM_EF_SML_LID_EXT, 1, (void*)(&sml_buffer_ext), sizeof(nvram_sml_context_struct_ext));
	sml_static_buffer_ext = &sml_buffer_ext;
	//sml_type_enum type = (sml_type_enum)sml_static_buffer_ext.sml_type[0];
	//kal_prompt_trace(MOD_SIM,"yangfengfeng:sml_type_enum type = %d", type);
	//if(type == SML_SIM1_SIM2_UNLOCK){ASSERT(0);}
}
*/
void hq_simlock_get_SML_EXT(nvram_sml_context_struct_ext *sml_buffer_ext)
{
	memset(sml_buffer_ext,0x00,sizeof(nvram_sml_context_struct_ext));
	kal_prompt_trace(MOD_SIM,"yangfengfeng:hq_simlock_fill_in_SML_EXT");
	nvram_external_read_data(NVRAM_EF_SML_LID_EXT, 1, (kal_uint8 *)(sml_buffer_ext), sizeof(nvram_sml_context_struct_ext));
}
void hq_simlock_set_SML_EXT(nvram_sml_context_struct_ext *sml_buffer_ext)
{
	nvram_external_write_data(NVRAM_EF_SML_LID_EXT, 1, (void*)(sml_buffer_ext), sizeof(nvram_sml_context_struct_ext));
}

#endif

#if 0//def __HQ_SML_LOCK_NV_ORANGE__
void orange_simlock_clear_gid1(void)
{
	gid1_context_struct gid1_mccmnc_buffer;
	nvram_external_read_data(NVRAM_EF_GID1_LID,1, (void*)(&gid1_mccmnc_buffer), sizeof(gid1_context_struct));
	gid1_mccmnc_buffer.change = 0x00;
	gid1_mccmnc_buffer.gid1[0] = 0x00;
	nvram_external_write_data(NVRAM_EF_GID1_LID, 1, (void*)(&gid1_mccmnc_buffer), sizeof(gid1_context_struct));
}
kal_uint8 orange_simlock_get_gid1_flag(void)
{
	gid1_context_struct gid1_buffer;
	nvram_external_read_data(NVRAM_EF_GID1_LID,1, (void*)(&gid1_buffer),  sizeof(gid1_context_struct));
	return gid1_buffer.change;
}
void orange_simlock_set_gid1(kal_uint8* gid1)
{
	S16 NumOfPlmn;
	S16 i ,error;
	sml_context_struct sml_static_buffer_o;
	nvram_external_read_data(NVRAM_EF_SML_LID,1, (void*)(&sml_static_buffer_o), NVRAM_EF_SML_SIZE);
	NumOfPlmn = sml_static_buffer_o.cat[SML_CAT_SP].num;
	for(i = 0; i < NumOfPlmn; i++)
	{
		sml_static_buffer_o.code_cat_sp[i*SML_SIZE_OF_CAT_SP + SML_SIZE_OF_CAT_N] = gid1[0];
	}
	nvram_external_write_data(NVRAM_EF_SML_LID, 1, (void*)(&sml_static_buffer_o), NVRAM_EF_SML_SIZE);
}
#endif
/*****************************************************************************
 * FUNCTION
...
#endif
#ifdef __HQ_SML_LOCK_NV_ANTI_BREAK__ 
    hq_simlock_fill_in_SML();
#endif
#ifdef __HQ_SML_LOCK_NV_EXT__
    //hq_simlock_fill_in_SML_EXT();
#endif
    return KAL_TRUE;
